...
export function registerRequestRoutes(
  ...
  app.get("/requests/stats", async (request, reply) => {
    ...
    // 1) Jellyseerr — source de vérité (paginer toutes les demandes du user)
    if (config) {
      try {
    ...
    // 2) Locales en attente (pas encore envoyées à Jellyseerr ou échouées)
  ...
  app.get("/requests", async (request, reply) => {
    ...
    // 1) Demandes locales en attente (pas encore envoyées à Jellyseerr ou échouées)
    ...
    // 2) Map locale par seerrRequestId pour enrichissement
    ...
    // 3) Demandes Jellyseerr — source de vérité
    // Si un filtre status/type est actif, on doit fetch toutes les demandes Jellyseerr
    // (sinon on ne voit que les premières 40 + on filtre dedans → résultats incomplets)
    ...
    try {
      ...
      // Détails TMDB en parallèle (cache par mediaType-tmdbId)
    ...
    // 4) Fusion : locales en attente d'abord, puis Jellyseerr (dédoublonné par seerrRequestId)
    ...
    let merged: UnifiedRequest[] = [...localFiltered, ...seerrUnified];
