...
- **Nouvelle page Sorties** : un vrai agenda, en vue **semaine** (sept jours, affiche et titre lisibles sans survol) ou **mois** (les titres s'affichent dans les cases, pas de simples pastilles). Trois modes : vos demandes en attente, tout ce qui sort, ou une plateforme entière (Crunchyroll, Netflix, Disney+, ADN, Canal+… 80 disponibles en France). Un titre peut être demandé directement depuis l'agenda
...
- **New Releases page**: a real calendar, in **week** view (seven days, poster and title readable without hovering) or **month** view (titles show inside the cells, not just coloured dots). Three modes: your pending requests, everything coming out, or a whole platform (Crunchyroll, Netflix, Disney+, ADN, Canal+… 80 available in France). Titles can be requested straight from the calendar
